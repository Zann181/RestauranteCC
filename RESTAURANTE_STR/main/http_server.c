...
/* An HTTP POST handler for setting the second RGB LED */
esp_err_t set_rgb2_handler(httpd_req_t *req) {
    ...
    content[ret] = 0; // Null-terminate the content string
    ESP_LOGI(TAG, "Received data for RGB2: %s", content);
	...
	if (cJSON_IsString(red) && cJSON_IsString(green) && cJSON_IsString(blue)) {
			...
			rgb_led_set_color2(red_value, green_value, blue_value);
			httpd_resp_send(req, "LED2 color set", HTTPD_RESP_USE_STRLEN);
		} else {
			ESP_LOGE(TAG, "Error parsing RGB2 values");
			httpd_resp_send_500(req);
		}
...
/* An HTTP GET handler to get the button press count */
esp_err_t get_button_count_handler(httpd_req_t *req) {
    char response[200]; // Asegúrate de que el buffer es lo suficientemente grande
    int count1 = button1_get_press_count();
    int count2 = button2_get_press_count();

    // Formato de la respuesta como JSON
    snprintf(response, sizeof(response), "{\"entradaDeCliente\": %d, \"salidaDeCliente\": %d}", count1, count2);
...
/* Manejador HTTP para obtener la temperatura */
esp_err_t get_temperature_handler(httpd_req_t *req) {
    ...
    if (xQueueReceive(adc_queue, &temperature, portMAX_DELAY)) {
        ...
        httpd_resp_send(req, temperature_str, HTTPD_RESP_USE_STRLEN);
        return ESP_OK;
    } else {
        httpd_resp_send_500(req);
        return ESP_FAIL;
...
*/
		...
		httpd_uri_t set_rgb_uri2 = {
			.uri       = "/setRGB2",
			.method    = HTTP_POST,
			.handler   = set_rgb2_handler,
			.user_ctx  = NULL
		};
		// Durante la configuración del servidor HTTP
		httpd_register_uri_handler(http_server_handle, &set_rgb_uri2);


		
		// Agrega una ruta para obtener el valor del contador del botón
		httpd_uri_t get_button_count_uri = {
			.uri      = "/get_button_count",
			.method   = HTTP_GET,
			.handler  = get_button_count_handler,
			.user_ctx = NULL
		};
		httpd_register_uri_handler(http_server_handle, &get_button_count_uri);
		...
		httpd_register_uri_handler(http_server_handle, &temperature_uri);
